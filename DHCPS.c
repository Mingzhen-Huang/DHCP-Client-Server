...
{
	dhcp_msg* Dmessage,*Omessage,*Amessage;
	int time;
	...
	//while(1){
		...
		sock=setnbind();
		if(Omessage=receivePacket(sock)){

			sock=setnbind();
			seteth1(sock);

			sendPacket(DHCP_OFFER,sock,Omessage,0);
			printf("change1\n");
		
		}	
		sock=setnbind();
		if(Amessage=receivePacket(sock)){

			sock=setnbind();
			seteth1(sock);

			sendPacket(DHCP_ACK,sock,Amessage,0);
			printf("change2\n");
